...
InlineKeyboardButton(" Iniciar siesta", callback_data="nap_start"),
InlineKeyboardButton(" Terminar siesta", callback_data="nap_end"),
]])
def inline_night() -> InlineKeyboardMarkup:
return InlineKeyboardMarkup([[
InlineKeyboardButton(" Iniciar noche", callback_data="night_start"),
InlineKeyboardButton(" Terminar noche", callback_data="night_end"),
]])
...
InlineKeyboardButton(" Guardar menú", callback_data="menu_save"),
InlineKeyboardButton(" Eliminar", callback_data="menu_delete"),
]])
# =========================
# Persistencia
# =========================
...
return f" '{food_clean}' ya está en la lista."
...
lines = [
f" "",
Recomendación de sueño — {baby_name(chat_data)} ({months} meses)",
f" f" "",
Siestas diarias: {nap_min}h – {nap_max}h",
Sueño nocturno: {night_min}h – {night_max}h",
" Rangos por edad:",
...
lines.append(f" {ns['start'].strftime('%d/%m')} — {fmt_time(ns['start'])} avg_h = (total_night / len(night_sleeps)).total_seconds() / 3600
avg_status = sleep_status_emoji(avg_h, night_min, night_max)
lines.append(f" Promedio: {format_duration(total_night / len(night_sleeps))} {avg_st
...
if active_day_nap else f" Última siesta: {fmt_datetime(str_to_dt(chat_data.get('las
)
return "\n".join([
...
nap_state,
f" Siestas hoy: {format_duration(total_naps)} {nap_status} (rango {nap_min}h-{nap_m
night_state, "",
f" Próximo: {next_text}",
])
# =========================
...
start_dt = str_to_dt(item.get("start_time"))
if start_dt:
day_naps.append(f" {start_dt.strftime('%H:%M')} - {dt.strftime('%H:%M')} ({
elif t == "night_sleep_end":
start_dt = str_to_dt(item.get("start_time"))
if start_dt:
night_sleeps.append(f" {start_dt.strftime('%d/%m %H:%M')} - {dt.strftime('%
...
lines.append(f"\n Siestas ({format_duration(total_naps)} {nap_status} | rango {nap_min}
...
async def send_to_chat(context: ContextTypes.DEFAULT_TYPE, chat_id: int, text: str) -> await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=keyboard())
None:
# =========================
# Callbacks inline
# =========================
...
await query.edit_message_text(f" Menú:\n\n{menu}" if menu else "No hay menú guardad
elif data == "menu_save":
chat_data["pending_menu_input"] = True
save_data()
await query.edit_message_text(" Pega el menú semanal ahora:")
elif data == "menu_delete":
delete_weekly_menu(chat_data)
await query.edit_message_text(" Menú eliminado.")
# =========================
# Comandos
# =========================
...
chat_data = get_chat_state(update.effective_chat.id)
save_data()
await send_with_keyboard(update, f"Hola! Bot de {baby_name(chat_data)} activo. Usa /help
...
save_data()
months = baby_age_months(chat_data)
await send_with_keyboard(update, f"Fecha guardada. {baby_name(chat_data)} tiene {mont
except ValueError:
...
if not update.effective_chat:
return
await send_with_keyboard(update, build_status_text(get_chat_state(update.effective_chat.i
async def history_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
if not update.effective_chat:
return
await send_with_keyboard(update, build_today_history_text(get_chat_state(update.effective
async def weekly_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
if not update.effective_chat:
return
await send_with_keyboard(update, build_weekly_summary(get_chat_state(update.effective_cha
async def sleep_rec_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
if not update.effective_chat:
return
await send_with_keyboard(update, build_sleep_recommendation(get_chat_state(update.effecti
...
if not update.message:
return
await update.message.reply_text(" Menú semanal:", reply_markup=inline_menu())
# =========================
# Texto libre
# =========================
...
application.add_handler(MessageHandler(filters.Regex(f"^{BUTTON_STATUS}$"), status_comman
application.add_handler(MessageHandler(filters.Regex(f"^{BUTTON_HISTORY}$"), history_comm
application.add_handler(MessageHandler(filters.Regex(f"^{BUTTON_WEEKLY}$"), weekly_comman
...
application.add_handler(MessageHandler(filters.Regex(f"^{BUTTON_SLEEP_REC}$"), sleep_rec_
application.add_handler(MessageHandler(filters.Regex(f"^{BUTTON_TRANSITION}$"), transitio
