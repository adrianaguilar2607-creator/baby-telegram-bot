...
BUTTON_MENU = "🗓️ Menú semanal"
BUTTON_DELETE_MENU = "🗑️ Eliminar menú"
...
def keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        [
            ...
            [BUTTON_WEEKLY],
            [BUTTON_MENU, BUTTON_DELETE_MENU],
        ],
...
def last_completed_day_nap(chat_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return naps[-1] if naps else None
...
def set_weekly_menu(chat_data: Dict[str, Any], menu_text: str) -> None:
    chat_data["weekly_menu"] = menu_text.strip()
    save_data()
...
def build_weekly_summary(chat_data: Dict[str, Any]) -> str:
    ...
    if night_sleeps:
        ...
        for ns in night_sleeps:
            ...
            lines.append(f"  {ns['start'].strftime('%d/%m')} — {fmt_time(ns['start'])} a {fmt_time(ns['end'])} ({format_duration(ns['duration'])})")
        lines.append(f"  Promedio: {format_duration(total_night / len(night_sleeps))}")
    ...
    if daily_naps:
        ...
        lines.append(f"  Promedio diario: {format_duration(total_nap_week / len(daily_naps))}")
...
def build_status_text(chat_data: Dict[str, Any]) -> str:
    ...
    night_state = f"🌙 Sueño nocturno en curso desde {fmt_time(active_night_sleep)}" if active_night_sleep else "🌙 Sueño nocturno: no activo"
...
def build_today_history_text(chat_data: Dict[str, Any]) -> str:
    ...
    for item in events:
        ...
        if event_type == "feed":
            ...
            start_dt = str_to_dt(item.get("start_time"))
            if start_dt:
                night_sleep.append(f"🌙 {start_dt.strftime('%d/%m %H:%M')} - {dt.strftime('%d/%m %H:%M')} ({format_duration(dt - start_dt)})")
    ...
    lines = ["📅 Historial de hoy", "", "🍼 Comidas:"]
    lines.extend(feeds if feeds else ["—"])
    lines.append(f"\n😴 Siestas del día ({format_duration(total_day_nap_today(chat_data))}):")
    lines.extend(day_naps if day_naps else ["—"])
    lines.append("\n🌙 Sueño nocturno:")
    lines.extend(night_sleep if night_sleep else ["—"])
    return "\n".join(lines)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    save_data()
    await send_with_keyboard(update, f"Hola. Ya guardé este chat para los recordatorios de {baby_name(chat_data)}.\n\nUsa /help para ver comandos.")
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "/status\n/history\n"
        "/addfood Nombre\n/foods\n/weekly\n"
        "/menu — ver o guardar menú semanal\n"
        "/deletemenu — eliminar menú semanal"
    )
...
async def napstart_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat:
        return
    chat_data = get_chat_state(update.effective_chat.id)
    dt = parse_manual_time(context.args[0]) if context.args else now_local()
    if context.args and dt is None:
...
async def addfood_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        ...
        return
    result = add_food(chat_data, " ".join(context.args).strip())
...
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if menu:
        await send_with_keyboard(update, f"🗓️ Menú semanal de {baby_name(chat_data)}:\n\n{menu}")
    else:
        chat_data["pending_menu_input"] = True
        save_data()
        await send_with_keyboard(update, "🗓️ No hay menú guardado. Pega el menú semanal ahora:")


async def deletemenu_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat:
        return
    chat_data = get_chat_state(update.effective_chat.id)
    if not get_weekly_menu(chat_data):
        await send_with_keyboard(update, "⚠️ No hay ningún menú guardado.")
        return
    delete_weekly_menu(chat_data)
    await send_with_keyboard(update, "🗑️ Menú eliminado. Pulsa 🗓️ Menú semanal para añadir uno nuevo.")
...
async def button_nap_start_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat:
        return
    chat_data = get_chat_state(update.effective_chat.id)
    ok, message = start_day_nap(chat_data, now_local())
    await send_with_keyboard(update, message)
...
async def button_nap_end_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat:
        return
    chat_data = get_chat_state(update.effective_chat.id)
    ok, message = end_day_nap(chat_data, now_local())
    await send_with_keyboard(update, message)
...
async def button_night_start_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat:
        return
    chat_data = get_chat_state(update.effective_chat.id)
    ok, message = start_night_sleep(chat_data, now_local())
    await send_with_keyboard(update, message)
...
async def button_night_end_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat:
        return
    chat_data = get_chat_state(update.effective_chat.id)
    ok, message = end_night_sleep(chat_data, now_local())
    await send_with_keyboard(update, message)
...
async def button_delete_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await deletemenu_command(update, context)
...
async def unknown_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat_data.get("pending_menu_input"):
        ...
        set_weekly_menu(chat_data, menu_text)
        await send_with_keyboard(update, "✅ Menú semanal guardado. Pulsa 🗓️ Menú semanal para consultarlo.")
        return
...
async def periodic_checks(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for chat_key, chat_data in chats.items():
        ...
        if next_feed:
            ...
            if current_now >= feed_15_time and current_now < next_feed:
                if not reminders.get("feed_15_sent", False):
                    await send_to_chat(context, chat_id, f"🍼 En 15 min toca comida para {baby_name(chat_data)}.")
                    reminders["feed_15_sent"] = True
                    changed = True
            if current_now >= next_feed:
                if not reminders.get("feed_due_sent", False):
                    await send_to_chat(context, chat_id, f"🍼 Ya toca comida para {baby_name(chat_data)}.")
        ...
        if next_nap and not night_active:
            ...
            if current_now >= nap_15_time and current_now < next_nap:
                if not reminders.get("nap_15_sent", False):
                    await send_to_chat(context, chat_id, f"😴 En 15 min toca siesta para {baby_name(chat_data)}.")
                    reminders["nap_15_sent"] = True
                    changed = True
            if current_now >= next_nap:
                if not reminders.get("nap_due_sent", False):
                    await send_to_chat(context, chat_id, f"😴 Ya toca siesta para {baby_name(chat_data)}.")
        ...
        tummy_dt = datetime(current_now.year, current_now.month, current_now.day, TUMMY_TIME_HOUR, TUMMY_TIME_MINUTE, tzinfo=TIMEZONE)
        if current_now >= tummy_dt and not daily.get("tummy_time_sent", False):
            await send_to_chat(context, chat_id, f"🤸 Tummy time de la mañana para {baby_name(chat_data)}.")
            daily["tummy_time_sent"] = True
            changed = True

        calm_dt = datetime(current_now.year, current_now.month, current_now.day, CALM_DOWN_HOUR, CALM_DOWN_MINUTE, tzinfo=TIMEZONE)
        if current_now >= calm_dt and not daily.get("calm_down_sent", False):
            await send_to_chat(context, chat_id, f"🌙 Hora de bajar actividad por la tarde para {baby_name(chat_data)}.")
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("deletemenu", deletemenu_command))
    ...
    application.add_handler(MessageHandler(filters.Regex(f"^{BUTTON_DELETE_MENU}$"), button_delete_menu))
